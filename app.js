...
function getRandomTip() {
    ...
    return londonTipsArray[randIdx]
}
...
app.get('/london-tips', (req, res) => {
    res.send(londonTipsArray)
});
// To do: Create a route for retrieving a random quote
app.get('/london-tips/random', (req, res) => {
    res.send(getRandomTip())
});
// To do: Refine this route and add handling for out-of-range IDs
//This was not working because of a typo
app.get('/london-tips/:id', (req, res) => {
    console.log(req.query);
    const tipId = req.params.id - 1;
    const selectedTip = londonTipsArray[tipId];
    res.send(selectedTip)
})

module.exports = app;


// FORMAT TO ADD IN THE HTML AND LINK TO THE BACKEND
/*
<link rel="stylesheet" href="assets/style.css">
  <script defer src="assets/script.js"></script>
</link>Get random quote flow
const getRandomQuote = async(e) => {
  e.preventDefault()
  try {
  const res = await fetch('http://localhost:3000/quotes/random%27);
...
//Binding event listener to form
